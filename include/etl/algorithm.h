...
namespace Project::etl {
    ...
    find(const Container& cont, const T& value) { return find(begin(cont), end(cont), value); }
    ...
    find_if(Container& cont, UnaryPredicate fn) { return find_if(begin(cont), end(cont), fn); }
    ...
    template <class Container, class UnaryPredicate> constexpr auto
    find_if_not(Container& cont, UnaryPredicate fn) { return find_if_not(begin(cont), end(cont), fn); }
    ...
    all(const Container& cont, const T& value) { return all(begin(cont), end(cont), value); }
    ...
    template <class Container, class T> constexpr bool
    any(const Container& cont, const T& value) { return any(begin(cont), end(cont), value); }
    ...
    template <class Container, class T> constexpr bool
    none(const Container& cont, const T& value) { return none(begin(cont), end(cont), value); }

    template <class Iterator, class UnaryPredicate> constexpr bool
    all_if(Iterator first, Iterator last, UnaryPredicate fn) { return find_if_not(first, last, fn) == last; }
    template <class Container, class UnaryPredicate> constexpr bool
    all_if(Container& cont, UnaryPredicate fn) { return all_if(begin(cont), end(cont), fn); }

    template <class Iterator, class UnaryPredicate> constexpr bool
    any_if(Iterator first, Iterator last, UnaryPredicate fn) { return find_if(first, last, fn) != last; }
    template <class Container, class UnaryPredicate> constexpr bool
    any_if(Container& cont, UnaryPredicate fn) { return any_if(begin(cont), end(cont), fn); }

    template <class Iterator, class UnaryPredicate> constexpr bool
    none_if(Iterator first, Iterator last, UnaryPredicate fn) { return find_if(first, last, fn) == last; }
    template <class Container, class UnaryPredicate> constexpr bool
    none_if(Container& cont, UnaryPredicate fn) { return none_if(begin(cont), end(cont), fn); }
    ...
    foreach(Container& cont, UnaryFunction fn) { return foreach(begin(cont), end(cont), fn); }
    ...
    fold(Container& cont, UnaryFunction fn, R& result) { return fold(begin(cont), end(cont), fn, result); }
    
    /// copy-assigns the given value to every element in a range
    template <class Iterator, class T> constexpr void
    fill(Iterator first, Iterator last, const T& value) { for (; first != last; ++first) *first = value; }
    template <class Container, class T> constexpr void
    fill(Container& cont, const T& value) { for (auto& x : cont) x = value; }
    ...
    generate(Container& cont, Generator fn) { for (auto& x : cont) x = fn(); }
    ...
    count(const Container& cont, const T& value) { return count(begin(cont), end(cont), value); }
    ...
    count_if(Container& cont, UnaryPredicate fn) { return count_if(begin(cont), end(cont), fn); }
    ...
    copy(const Container& cont, ContainerDest& dest) { for (auto [x, y] : zip(cont, dest)) y = x; }
    ...
    template <class Container, class ContainerDest> constexpr void
    move(const Container& cont, ContainerDest& dest) { for (auto [x, y] : zip(cont, dest)) y = move(x); }
    ...
    swap(Container1& cont1, Container2& cont2) { for (auto [x, y] : zip(cont1, cont2)) swap(x, y); }
    ...
    template <class Container, class T> constexpr void
    replace(Container& cont, const T& old, const T& value) { replace(begin(cont), end(cont), old, value); }
    ...
    replace_if(Container& cont, UnaryPredicate fn, const T& value) { replace_if(begin(cont), end(cont), fn, value); }
    ...
    max_element(const Container& cont) { return max_element(begin(cont), end(cont)); }
    ...
    template <class Container> constexpr auto
    min_element(const Container& cont) { return min_element(begin(cont), end(cont)); }
